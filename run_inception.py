...
import inception_architecture as model               
...
tf.app.flags.DEFINE_integer('epoch', 2, "training epoch")
#tf.app.flags.DEFINE_integer('type_', 50, "type of resnet")
tf.app.flags.DEFINE_float('test_size',0.1,'test size')
tf.app.flags.DEFINE_float('val_size',0.1,'val size')
tf.app.flags.DEFINE_integer('train_size',100,'training size')
...
tf.app.flags.DEFINE_string('save_name','saved_resnet_model','folder of saving')
tf.app.flags.DEFINE_integer('validation_interval',1,'validation_interval')
...
def train():
	...
	X,y,learning_rate,dropout_keep_prob,training=model.placeholders(img_size,img_channel,label_cnt)   
	logits1,logits2,logits3,out_probs=model.network(X,dropout_keep_prob=dropout_keep_prob,label_cnt=label_cnt,training=training)
	loss=model.loss(logits1,logits2,logits3,labels=y)
	...
	for epoch in range(epochs):
		...
		for batch in range(num_batches):
			...
			if i%20==0:
				...
				_,_,batch_loss=sess.run([optimizer,loss],feed_dict={X:train_x,y:train_y,learning_rate:lr,dropout_keep_prob:kp,training:True})
			...
			if i%FLAGS.validation_interval==0 and i>0:
				...
				accuracy_batch,batch_loss,summary=sess.run([accuracy,loss,summary],feed_dict={X:val_images,y:val_labels,dropout_keep_prob:1.0,training:False})
